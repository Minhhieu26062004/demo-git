...
<body>
    ...
    $ID = $_GET['ID'];
    $sqlSelect = "SELECT * FROM `tbl_nhanvien` WHERE ID='".$ID."';";
    ...
    <header class="err">
        ...
        <section class="section bg-white p-6 rounded-lg shadow-lg">
            <h2 class="text-2xl font-bold mb-4">Chỉnh sửa Nhân Viên</h2>
            <form>
            
                <div class="form-group">
                    <label for="product-name">Tên NV</label>
                    <input id="tennhanvien" name="tennhanvien" placeholder="Nhập tên nhân viên" type="text" value="<?php echo $Khoa_Row['tennhanvien'] ?>"/>
                </div>
                <div class="form-group">
                    <label for="product-price">Địa chỉ</label>
                    <input id="diachi" name="diachi" placeholder="Nhập địa chỉ" type="text" value="<?php echo $Khoa_Row['diachi'] ?>" />
                </div>
                <div class="form-group">
    ...
    if (isset($_POST['btnSave'])) {
        ...
        // Câu lệnh UPDATE
        $sql = "UPDATE tbl_nhanvien SET tennhanvien='$tennhanvien',diachi='$diachi',anhmota='$anhmota'  WHERE ID='".$ID."';";
